...
for host in hosts:
  ...
  for line in out.splitlines():
      if (re.search(docker_stack, line.decode('utf-8'))):
          ...
          if (not re.search('Paused',line.decode('utf-8'))):
              # When not paused
              fields = line.decode('utf-8').strip().split()
              container = fields[0]
              print ('Running container: ' + container + ' found.  Pausing now.')
              # Pause the container
              out = subprocess.Popen(['docker','-H','os'+host+':2375','pause', container])
